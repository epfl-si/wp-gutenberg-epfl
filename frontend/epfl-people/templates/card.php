    ...
    function epfl_people_card($persons, $from, $columns) {
        ...
        foreach($persons as $index => $person) {
            ...
            if ($photo_url) {
                ...
                $photo_url = bloginfo('template_url') . '/assets/images/defaults/person-avatar-default-small.png';
            }
            ...
            $markup .= '</div>';
            $markup .= '<h3><a class="link-pretty" href="<?php echo esc_url($people_url) ?>"><?php echo esc_html($person->prenom) ?> <?php echo esc_html($person->nom) ?></a></h3>';
            $markup .= '<dl class="definition-list definition-list-grid mb-0">';
            
            if ($function) {
                $markup .= '<dt>' . esc_html('Position', 'epfl') . '</dt>';
            ...
            if ($room) {
                $markup .= '<dt>' . esc_html('Office', 'epfl') . '</dt>';
            ...
            if ($person->email) {
                $markup .= '<a class="btn btn-block btn-primary mb-2" href="mailto:<?php echo esc_attr($person->email) ?>"><?php echo esc_html($person->email) ?></a>';
            }
            if ($phones[0]) {
                $markup .= '<a class="btn btn-block btn-secondary" href="tel:+412169<?php echo esc_html($phones[0]) ?>">+41 21 69 <?php echo esc_html($phones[0]) ?></a>';
            }
            $markup .= '</div>';
            $markup .= '</div>';
            $markup .= '</div>';
