...
Class DirectorAuthorInfoscienceField2018Render extends AuthorInfoscienceField2018Render {
    public static function render($publication, $format, $has_next=false, $field_name='author') {
        ...
        if (self::field_exists($publication['director'])) {
            if ($format === 'detailed')
            {
                $html_rendered .= " " . __('Director(s)', 'epfl') . " : ";
                $html_rendered .= self::render_authors($publication['director']);
            } else {
                $html_rendered .= self::render_authors($publication['director']);
                $html_rendered .= " (" . __('Dir.', 'epfl') . ") ";
            }
        }
...
Class BooksChaptersPublicationLocationInsitutionDateInfoscienceField2018Render extends InfoscienceField2018Render {
    public static function render($publication, $format, $has_next=false, $institution_separator = ': ') {
        $html_rendered = "";
...
Class JournalPageInfoscienceField2018Render extends InfoscienceField2018Render {
    public static function render($publication, $format, $has_next=false) {
        ...
        if (self::field_exists($publication['journal'], 'page')) {
            if ($format === 'detailed') {
                $html_rendered .= '<p class="mb-0 infoscience_journal_page">' . __('p.', 'epfl') . ' ' . $publication['journal'][0]['page'] .'</p>';
            } else {
                $html_rendered .= ' <span class="mb-0 infoscience_journal_page">' . __('p.', 'epfl') . ' ' . $publication['journal'][0]['page'] .'. </span>';
            }
        }
    ...
    public static function render($publication, $format, $has_next=false) {
        ...
        if ($format === 'detailed') {
            if (self::field_exists($publication['journal'], 'volume')) {
                $html_rendered .= '<span class="infoscience_journal_volume">' . __('Vol.', 'epfl') . ' ' . $publication['journal'][0]['volume'] .' ';
            ...
            if (self::field_exists($publication['journal'], 'number')) {
                $html_rendered .= '<span class="infoscience_journal_number">' . __('num.', 'epfl') . ' ' . $publication['journal'][0]['number'] .'';
                if (self::field_exists($publication['journal'], 'page')) {
                    $html_rendered .= '</span>, ';
                } else {
                    $html_rendered .= '</span>. ';
            ...
            if (self::field_exists($publication['journal'], 'number')) {
                $html_rendered .= '<span class="text-muted small mb-0 infoscience_journal_number">' . __('num.', 'epfl') . ' ' . $publication['journal'][0]['number'];
                if (self::field_exists($publication['journal'], 'page')) {
                    $html_rendered .= '</span>, ';
                } else {
                    $html_rendered .= '</span>. ';
    ...
    public static function render($publication, $format, $has_next=false) {
        ...
        if (self::field_exists($publication['patent'])) {
            if ($format === 'detailed') {
                ...
                $html_rendered .= __('Patent number(s)', 'epfl') . " :<br />";
...
Class PublicationPageInfoscienceField2018Render extends InfoscienceField2018Render {
    public static function render($publication, $format, $has_next=false) {
        ...
        if (self::field_exists($publication['publication_page'])) {
            $html_rendered .= '<span class="mt-2 mb-2 infoscience_publication_page">' . __('p.', 'epfl') . ' ' . $publication['publication_page'][0] .'.</span> ';
        }
        return $html_rendered;
    }
}
