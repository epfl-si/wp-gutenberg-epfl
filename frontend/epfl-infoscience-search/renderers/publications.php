...
abstract Class InfosciencePublication2018Render {
    ...
    protected static function render_links($publication) {
        ...
        if ($fulltext || $doi) {
            ...
            if ($fulltext) {
                $links_html .= '<a class="text-muted" href="' . esc_attr($fulltext) . '" target="_blank">' .  esc_html__('Full text', 'epfl') . '</a>';
            ...
            if ($doi) {
                $links_html .= '<a class="text-muted" href="https://dx.doi.org/' . esc_attr($doi[0]) . '" target="_blank">' .  esc_html__('View at publisher', 'epfl') . '</a>';
            }
