...
function epfl_courses_block( $attributes ) {
    ...
    else
    {
        ...
        foreach($items as $item)
        {
            ...
            if(property_exists($item, 'X_OBJECTIFS') && $item->X_OBJECTIFS != "")
            {
                $desc = '<small style="margin:-12px 0px 32px 0px">'.utf8_decode($item->X_OBJECTIFS).'</small>';    
                $link_style = '';
            }
            else
            {
                $desc = '';
                $link_style = 'style="margin-bottom:32px"';
            }
            ...
            $html .= '<a class="nav-link nav-pretty" href="'.$item->X_URL.'" '.$link_style.'>'.utf8_decode($item->X_MATIERE).' '.$lang.'</a>'.$desc;
        }
